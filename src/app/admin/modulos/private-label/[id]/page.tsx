...
import { listProductionOrdersForProject, productionOrderStatuses } from "@/lib/private-label-production";
import {
  approveProjectProposalAction,
...
type OrderRow = { id: number; proposal_id: number; quantity: number; status: string; scheduled_date: string | null; notes: string | null; created_at: string | Date; updated_at: string | Date; product_name: string | null };
...
function orderStatus(value: string) {
  if (value === "em_producao") return "Em produção";
  if (value === "concluida") return "Concluída";
  if (value === "cancelada") return "Cancelada";
  if (value === "programada") return "Programada";
  return "Planejamento";
}
...
export default async function PrivateLabelProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [detail, orderRows] = await Promise.all([getPrivateLabelProjectDetail(projectId), listProductionOrdersForProject(projectId)]);
  if (!detail) notFound();
  ...
  return (
    <div className="space-y-7 pb-10">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <Metric icon={ClipboardCheck} label="Ordens de produção" value={orders.length} />
        <Metric icon={PackageCheck} label="Lotes" value={lots.length} />
      </section>

      <section className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.17em] text-red-700">Briefing consolidado</p><h2 className="mt-1 text-xl font-black text-slate-950">O que precisa ser desenvolvido</h2><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-600">{project.briefing || "O projeto ainda não possui resumo de briefing."}</p></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><UserRound size={19} className="text-red-700" /><h2 className="text-lg font-black text-slate-950">Cliente</h2></div><p className="mt-4 font-black text-slate-950">{project.brand_name}</p><p className="mt-1 text-sm text-slate-500">{project.legal_name}</p><div className="mt-4 space-y-1 text-sm text-slate-600"><p>{project.contact_name || "Responsável não informado"}</p><p>{project.email || "E-mail não informado"}</p><p>{project.phone || "Telefone não informado"}</p></div></div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[.8fr_1.2fr]">
          <form action={createProjectFormulaAction} className="rounded-2xl bg-slate-50 p-4"><input type="hidden" name="projectId" value={project.id} /><p className="font-black text-slate-950">Criar fórmula vinculada</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><Field label="Nome"><input className="field" name="name" required placeholder="Whey 900g" /></Field><Field label="Código"><input className="field" name="code" required placeholder="PL-WHEY-001" /></Field><Field label="Versão"><input className="field" name="version" defaultValue="v1" required /></Field><Field label="Categoria"><input className="field" name="category" /></Field><Field label="Unidades do lote"><input className="field" name="batchUnits" type="number" min="1" defaultValue="1000" required /></Field><Field label="Peso líquido (g)"><input className="field" name="netWeightG" type="number" min="0" step="0.01" placeholder="900" /></Field></div><Field label="Responsável"><input className="field" name="responsible" placeholder="Preenchido com seu nome se vazio" /></Field><button className="btn-primary mt-4 w-full justify-center" type="submit">Criar e iniciar engenharia</button></form>
          <div className="space-y-3">{formulas.length === 0 ? <Empty text="Nenhuma fórmula vinculada. Crie a primeira para iniciar a engenharia deste projeto." /> : formulas.map((formula) => <article key={formula.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><span className="rounded-full bg-slate-950 px-2.5 py-1 text-[10px] font-black uppercase text-white">{formula.status}</span><span className="text-xs font-bold text-slate-400">{formula.code} · {formula.version}</span></div><h3 className="mt-2 text-lg font-black text-slate-950">{formula.name}</h3><p className="mt-1 text-xs text-slate-500">{formula.batch_units.toLocaleString("pt-BR")} un. · {formula.net_weight_g ? `${formula.net_weight_g}g` : "peso a definir"} · {formula.responsible || "sem responsável"}</p></div><div className="text-right"><p className="text-xs font-bold text-slate-500">MP do lote</p><p className="font-black text-slate-950">{money(formula.raw_cost)}</p></div></div><div className="mt-4 grid grid-cols-3 gap-2"><Small label="Composição" value={`${Number(formula.total_percentage || 0).toFixed(2)}%`} /><Small label="Embalagem/un." value={money(formula.packaging_unit_cost)} /><Small label="Criada" value={date(formula.created_at)} /></div></article>)}</div>
        </div>
      </section>

      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.17em] text-red-700">2. Custo e margem</p><h2 className="mt-1 text-2xl font-black text-slate-950">Precificação industrial</h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.17em] text-red-700">3. Comercial</p><h2 className="mt-1 text-2xl font-black text-slate-950">Proposta pronta para decisão</h2>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-2">{proposals.length === 0 ? <div className="lg:col-span-2"><Empty text="Nenhuma proposta gerada para este projeto." /></div> : proposals.map((proposal) => <article key={proposal.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${proposal.status === "aprovada" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>{proposal.status}</span><h3 className="mt-2 font-black text-slate-950">{proposal.title}</h3><p className="mt-1 text-xs text-slate-500">Validade: {proposal.validity_days} dias · PDF {proposal.pdf_status} · {proposal.send_status}</p></div><FileText size={22} className="text-red-700" /></div><div className="mt-4 grid grid-cols-3 gap-2"><Small label="Quantidade" value={`${proposal.quantity.toLocaleString("pt-BR")} un.`} /><Small label="Preço/un." value={money(proposal.unit_price)} /><Small label="Total" value={money(proposal.total_price)} /></div><div className="mt-4 flex flex-wrap justify-end gap-2"><Link href={`/admin/modulos/private-label/propostas/${proposal.id}`} className="btn-secondary">Abrir proposta</Link>{proposal.status !== "aprovada" ? <form action={approveProjectProposalAction}><input type="hidden" name="projectId" value={project.id} /><input type="hidden" name="proposalId" value={proposal.id} /><button className="btn-primary" type="submit"><CheckCircle2 size={16} /> Aprovar e gerar OP</button></form> : <span className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700"><CheckCircle2 size={15} /> Comercial aprovado</span>}</div></article>)}</div>
      </section>

      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><ClipboardCheck size={20} className="text-red-700" /><div><p className="text-xs font-black uppercase tracking-[0.17em] text-red-700">4. PCP</p><h2 className="text-xl font-black text-slate-950">Ordens de produção</h2></div></div>
        <div className="mt-5 space-y-3">{orders.length === 0 ? <Empty text="A aprovação comercial gera automaticamente a ordem de produção e uma tarefa para Operações + PCP." /> : orders.map((order) => <article key={order.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-slate-950 px-2.5 py-1 text-[10px] font-black uppercase text-white">OP #{String(order.id).padStart(5, "0")}</span><span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-black uppercase text-amber-700">{orderStatus(order.status)}</span></div><h3 className="mt-2 font-black text-slate-950">{order.product_name || project.product_name || project.name}</h3><p className="mt-1 text-xs text-slate-500">{Number(order.quantity).toLocaleString("pt-BR")} unidades · criada em {date(order.created_at)}</p></div><form action={updateProjectProductionOrderAction} className="grid gap-2 sm:grid-cols-[160px_170px_1fr_auto]"><input type="hidden" name="projectId" value={project.id} /><input type="hidden" name="orderId" value={order.id} /><select name="status" defaultValue={order.status} className="field h-11 py-0">{productionOrderStatuses.map((value) => <option key={value} value={value}>{orderStatus(value)}</option>)}</select><input type="date" name="scheduledDate" defaultValue={order.scheduled_date || ""} className="field h-11 py-0" /><input name="notes" defaultValue={order.notes || ""} placeholder="Observações da programação" className="field h-11" /><button className="btn-primary" type="submit">Salvar PCP</button></form></div></article>)}</div>
      </section>

      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><Boxes size={20} className="text-red-700" /><div><p className="text-xs font-black uppercase tracking-[0.17em] text-red-700">5. Produção e rastreabilidade</p><h2 className="text-xl font-black text-slate-950">Lotes do produto</h2></div></div><div className="mt-5 grid gap-3 lg:grid-cols-2">{lots.length === 0 ? <div className="lg:col-span-2"><Empty text="Ainda não há lote produzido. O cadastro do lote encerra o ciclo operacional e disponibiliza a rastreabilidade pública." /></div> : lots.map((lot) => <article key={lot.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center justify-between"><div><p className="font-black text-red-700">{lot.code}</p><p className="mt-1 text-xs text-slate-500">{lot.quantity} {lot.unit} · fabricação {lot.manufacturing_date} · validade {lot.expiration_date}</p></div><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black uppercase text-slate-600">{lot.status}</span></div></article>)}</div><div className="mt-4 flex justify-end"><Link href="/admin/lotes" className="btn-secondary">Cadastrar / gerenciar lote</Link></div></section>

      <section className="rounded-[24px] border border-emerald-200 bg-gradient-to-r from-emerald-50 to-white p-5"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-emerald-600" size={22} /><div><p className="font-black text-emerald-950">Fluxo conectado</p><p className="mt-1 text-sm leading-6 text-emerald-900/70">Briefing, cliente, produto, fórmula, materiais, embalagem, lote econômico, precificação, proposta, aprovação, ordem de produção e rastreabilidade compartilham o mesmo DNA do projeto.</p></div></div></section>
    </div>
  );
}

function Metric({ icon: Icon, label, value }: { icon: typeof FlaskConical; label: string; value: number }) { return <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div className="grid h-10 w-10 place-items-center rounded-xl bg-slate-950 text-white"><Icon size={18} /></div><span className="text-2xl font-black text-slate-950">{value}</span></div><p className="mt-3 text-sm font-bold text-slate-600">{label}</p></div>; }

...
import {
  addFormulaItem, addFormulaPackagingItem, approveEngineeringFormula, createClient, createEngineeringFormula,
  createEngineeringProject, createEngineeringSupplier, createLot, createPackagingMaterial, createProduct,
  createProposalFromPricing, createRawMaterial, duplicateEngineeringFormula, sendFormulaToPricing
} from "@/lib/repository";
...
export async function duplicateEngineeringFormulaAction(formData: FormData) { const session = await assertWorkspaceWrite("engenharia"); const id = numberValue(formData, "formulaId"); await duplicateEngineeringFormula(id); await recordWorkspaceAudit({ actorMemberId: session.member.id, action: "formula.duplicated", entityType: "formula", entityId: id, summary: `${session.member.name} duplicou a fórmula #${id}.` }); revalidatePath("/admin/engenharia"); }
export async function approveEngineeringFormulaAction(formData: FormData) { const session = await assertWorkspaceWrite("engenharia"); const id = numberValue(formData, "formulaId"); await approveEngineeringFormula(id); await recordWorkspaceAudit({ actorMemberId: session.member.id, action: "formula.approved", entityType: "formula", entityId: id, summary: `${session.member.name} aprovou a fórmula #${id}.` }); revalidatePath("/admin/engenharia"); }
...
export async function addFormulaPackagingItemAction(formData: FormData) { const session = await assertWorkspaceWrite("engenharia"); const formulaId = numberValue(formData, "formulaId"); await addFormulaPackagingItem({ formulaId, packagingMaterialId: numberValue(formData, "packagingMaterialId"), quantity: numberValue(formData, "quantity") || 1 }); await recordWorkspaceAudit({ actorMemberId: session.member.id, action: "formula.packaging_added", entityType: "formula", entityId: formulaId, summary: `${session.member.name} adicionou embalagem à fórmula #${formulaId}.` }); revalidatePath("/admin/engenharia"); }
export async function sendFormulaToPricingAction(formData: FormData) { const session = await assertWorkspaceWrite("engenharia"); const formulaId = numberValue(formData, "formulaId"); await sendFormulaToPricing({ formulaId, projectId: numberValue(formData, "projectId") }); await recordWorkspaceAudit({ actorMemberId: session.member.id, action: "pricing.created", entityType: "formula", entityId: formulaId, summary: `${session.member.name} enviou a fórmula #${formulaId} para precificação.` }); revalidatePath("/admin/engenharia"); }
export async function createProposalFromPricingAction(formData: FormData) { const session = await assertWorkspaceWrite("engenharia"); const pricingId = numberValue(formData, "pricingRequestId"); await createProposalFromPricing(pricingId); await recordWorkspaceAudit({ actorMemberId: session.member.id, action: "proposal.created", entityType: "pricing_request", entityId: pricingId, summary: `${session.member.name} gerou proposta a partir da precificação #${pricingId}.` }); revalidatePath("/admin/engenharia"); }

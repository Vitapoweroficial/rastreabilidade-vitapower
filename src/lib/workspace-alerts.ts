...
function normalizeStatus(value: string | undefined): WorkspaceTaskStatus {
  return workspaceTaskStatuses.includes(value as WorkspaceTaskStatus) ? value as WorkspaceTaskStatus : "novo";
}
...
export async function listWorkspaceAlerts(limit = 20) {
  await ensureWorkspaceAlertSchema();
  const rows = await getSql().query(
    `SELECT id, source_key, type, audience, title, message, href, entity_type, entity_id, read_at,
            task_status, assignee, due_at, accepted_at, completed_at, created_at, updated_at
     FROM workspace_alerts
     ORDER BY (task_status <> 'concluido') DESC,
              (due_at IS NOT NULL AND due_at < NOW() AND task_status <> 'concluido') DESC,
  ...
  return rows.map((row) => ({
    ...
    taskStatus: normalizeStatus(row.task_status),
...
export async function getOpenWorkspaceTaskCount() {
  await ensureWorkspaceAlertSchema();
  const rows = await getSql().query(`SELECT COUNT(*)::int AS count FROM workspace_alerts WHERE task_status <> 'concluido'`) as unknown as { count: number }[];
  return Number(rows[0]?.count ?? 0);
}
...
export async function markAllWorkspaceAlertsRead() {
  await ensureWorkspaceAlertSchema();
  await getSql().query(`UPDATE workspace_alerts SET read_at = NOW(), updated_at = NOW() WHERE read_at IS NULL`);
}
...
export async function updateWorkspaceTask(input: {
  ...
  const currentRows = await sql.query(
    `SELECT task_status, assignee, due_at FROM workspace_alerts WHERE id = $1`,
    [input.alertId]
  ) as unknown as { task_status: WorkspaceTaskStatus; assignee: string | null; due_at: string | Date | null }[];
  ...
  const assignee = input.assignee === undefined ? current.assignee : (input.assignee?.trim() || null);
  ...
  await sql.query(
    `UPDATE workspace_alerts SET
       ...
       updated_at = NOW()
     WHERE id = $1`,
    [input.alertId, status, assignee, dueAt]
  );

  const rows = await sql.query(
    `SELECT id, source_key, type, audience, title, message, href, entity_type, entity_id, read_at,
            task_status, assignee, due_at, accepted_at, completed_at, created_at, updated_at
     FROM workspace_alerts WHERE id = $1`,
    [input.alertId]
  ) as unknown as AlertRow[];
  const row = rows[0];
  return {

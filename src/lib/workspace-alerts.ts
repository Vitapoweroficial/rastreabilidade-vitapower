...
export async function ensureWorkspaceAlertSchema() {
  if (!alertSchemaPromise) {
    alertSchemaPromise = (async () => {
      ...
      await sql.query(`
        UPDATE workspace_alerts a
        SET assignee_member_id = m.id
        FROM workspace_members m
        WHERE a.assignee_member_id IS NULL
          AND a.assignee IS NOT NULL
          AND LOWER(TRIM(a.assignee)) = LOWER(TRIM(m.name))
      `);
      await Promise.all([
...
export async function updateWorkspaceTask(input: {
  ...
  const currentRows = await sql.query(
    `SELECT task_status, assignee, assignee_member_id, due_at FROM workspace_alerts WHERE id = $1`,
    [input.alertId]
  ) as unknown as { task_status: WorkspaceTaskStatus; assignee: string | null; assignee_member_id: number | null; due_at: string | Date | null }[];

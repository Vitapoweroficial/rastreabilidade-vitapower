...
const seed = db.transaction(() => {
  db.exec(`
    ...
    DELETE FROM clients;
    DELETE FROM sqlite_sequence WHERE name IN ('clients', 'products', 'lots');
  `);
  ...
  insertLot.run(
    ...
    "Lote aprovado nos parâmetros físico-químicos, microbiológicos e sensoriais.",
    "Matérias-primas conferidas contra ordem de produção OP-2026-044."
  );
  ...
  insertLot.run(
    ...
    "Aguardando conclusão da análise microbiológica.",
    "Retenção de amostras realizada em 2026-05-04."
  );
});
